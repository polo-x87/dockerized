...
**Share workflow with AI agents on this Mac:**

- Open `dockerize.code-workspace` (multi-root: dockerize + vault + code) or folder `~/vault/mcp/dockerize` so `.cursor/rules/dockerize.mdc` applies.
- Vault-wide pointer: `~/vault/.cursor/rules/devbox-pointer.mdc` (applies when workspace is under `~/vault` or `~/code`).
- MCP on host: `scripts/sync-cursor-mcp.sh` → `~/.cursor/mcp.json`.
- Claude Code: `CLAUDE.md` at repo root. Gemini CLI: `GEMINI.md`.
...
│   └── deepclaude/       claude-code-router overlay (Claude Code → free OpenRouter
│                         backend via ccr); OPENROUTER_API_KEY via op run
├── dotfiles/             personal host dotfiles mirrored from ~/. Drop-in
│                         alternative to base/dotfiles-sample (set DOTFILES_LOCAL
│                         in base/.env to use). Excludes anything with credentials.
├── scripts/
...
│   ├── sync-cursor-mcp.sh  repo mcp.json → ~/.cursor/mcp.json
│   └── seed-op-env.sh    op agent env templates → ~/.config/op/
├── setup.sh              (generated by docs/guide.html) one-shot bootstrap with
│                         per-var validation, auto-detect menu, and saved answers
├── .devbox.env           (auto-written by setup.sh) saved answers, sourced on
│                         next run; survives regeneration of setup.sh
└── docs/
    ├── guide.html        interactive guide that generates all user-facing artifacts
    ├── usage.md          daily workflows + troubleshooting (incl. setup.sh config flow)
...
## Contributing — docs sync rule

Every change to `base/`, `scripts/`, `overlays/`, or root configs MUST also update the docs in the same commit. This keeps all AI agents (Claude Code, Codex, Gemini) operating on a shared, accurate ground truth. See `CLAUDE.md` Hard rule #8 for the exact file list.

**Enforced by a pre-commit hook.** `scripts/install-host.sh` installs it automatically; to install manually: `scripts/install-git-hooks.sh`. Bypass when truly irrelevant with `SKIP_DOCS_CHECK=1 git commit …` or `git commit --no-verify`.

## See also

- `docs/usage.md` — daily workflows, AI agent setup, common problems
- `docs/guide.html` — interactive setup guide (source of truth for user-facing artifacts)
- `docs/cli-mesh.html` — master node-mesh of tools, configs, and bloat/error sources
